...
static inline void destroy_all_internal( unsigned int shm_idx )
{
    ...
    current = list->head;
    list->head = NULL;
    __atomic_store_n( &list->is_used, 0, __ATOMIC_SEQ_CST );
...
unsigned int msync_alloc_shm( int low, int high )
{
    ...
    while (__atomic_load_n( &mach_semaphore_map[shm_idx].is_used, __ATOMIC_SEQ_CST ))
    {
    ...
    __atomic_store_n( &mach_semaphore_map[shm_idx].is_used, 1, __ATOMIC_SEQ_CST );
